...
const ProductDetails = () => {
  ...
  const handeMouseEnterProduct = (imageURL)=>{
                  ...
                  data?.productImage?.map((imgURL,index) =>{
                    return(
                      <div className='h-20 w-20 bg-slate-200 rounded p-1' key={imgURL}>
                        <img src={imgURL} className='w-full h-full object-scale-down mix-blend-multiply cursor-pointer' onMouseEnter={()=>handeMouseEnterProduct(imgURL)} onClick={()=>handeMouseEnterProduct(imgURL)}/>
                      </div>
                    )
                  })
                }
        ...
        {/** Product Details */}
          ...
          <p className='capitalize text-slate-400 bg-slate-200 min-w-[100px] animate-pulse h-6 lg:h-8 w-full'></p>
           ...
           <div className='flex items-center gap-3 my-2 w-full'>
            ...
            <button className='h-10 lg:h-8 bg-slate-200 rounded animate-pulse w-full'></button>
           </div>

           <div className='w-full'>
            <p className='text-slate-600 font-medium my-1 h-6 lg:h-8 bg-slate-200 rounded animate-pulse w-full'></p>
            <p className='h-6 bg-slate-200 rounded animate-pulse lg:h-12 w-full'></p>
           </div>
              </div>
            ) : 
            (
